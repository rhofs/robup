...
import { workspaceIdForList } from '../../lib/workspaceMembers';
...
// several workspaces at once, and the team strip is one of them — and are not on it already. The
// same rule the pickers and the server apply; this just says "no" during the hover instead of after.
...
export function useTaskAssignDrop(task: Task) {
  ...
  return usePersonDrop(
    (uid) => !!ws?.members.some((m) => m.id === uid) && !task.assignees.some((a) => a.id === uid),
